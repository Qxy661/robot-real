...
> **从仿真到真机**——把 M2 检测、M5 导航、M6 控制、micro-ROS 底层集成到一台真实 ROS2 小车，跑通「感知 → 规划 → 控制 → 执行」完整闭环。
...
```
        【主线】能做事（巡检）→ 会自主（巡逻）→ 懂人话（语音）
                                   ▲
┌─────────────────────────────────────────────────────────────┐
│  ④ 系统层：三任务递进 + 自定义中控台                          │
├─────────────────────────────────────────────────────────────┤
│  ③ 规划层：手写 DWA/A* 真机 + frontier 探索                   │
├─────────────────────────────────────────────────────────────┤
│  ② 感知层：超声波专项 + 激光 SLAM + YOLO                     │
├─────────────────────────────────────────────────────────────┤
│  ① 控制层：里程计标定 + PID + 航向融合                       │
└─────────────────────────────────────────────────────────────┘
         ▲ 深化：检测导航闭环 → 手写算法真机 → K230 边缘 AI
```

四层递进：控制 → 感知 → 规划 → 系统，每层都是下一层的地基，每层都有量化产出支撑。

## 🏆 已完成成果

| 层 | 成果 | 结果 |
|---|---|---|
| 控制 | **micro-ROS 通信** | ESP32-S3 控制板 ↔ PC 跑通（/odom /imu /scan /cmd_vel）|
| 控制 | **控制层脚本 ×3** | 里程计标定 / PID 精确行驶 / IMU+里程计航向融合 |
...
- **能力分层递进** — 控制 → 感知 → 规划 → 系统，每一层都是下一层的地基
...
│   ├── control/           # 控制层：里程计标定 / PID 精确行驶 / IMU 航向融合
...
实机能力组织为四个递进层次，每层带量化产出，详见 [08-作品集框架·实机能力](docs/08-作品集框架-实机能力.md)。
...
控制层是底层功底，三个脚本见 [capabilities/control/README.md](capabilities/control/README.md)：
